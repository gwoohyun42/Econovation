						...
						handler : function(){
							...
							// id를 이용하여 formPanel에 입력한 값을 저장하는 방법 
                      	    ...
                      	    formPanel.submit({
                      	    		...
                      	    		// domain : protocol + host + path + queryString(parameter)
                      	    		params : [
	              			...
	              			Ext.data.JsonP.request({
	              				...
	              				callbackKey : 'callback',	// callback 이름, callback function과 다름
	              				params : [name,birthday,mili,career,gender,age,email,url,pay], 
	              				// 서버측 파라미터 수신시에시
	              				// 파라미터 순서대로 0, 1, 2, .... 
	              				// string name = request.getParameter("0");
	              				success : function(result){	// 서버 응답시 무조건 success
	              					Ext.Viewport.unmask();
	              				...
	              				failure : function(result){	// 서버 오류나 무응답시 failure
	              					Ext.Viewport.unmask();
